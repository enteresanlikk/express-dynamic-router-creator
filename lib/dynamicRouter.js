...
class DynamicRouter {
  constructor({ app, folders, routerFiles, info = true }) {
    this.app = app;
  ...
  validate() {
    ...
    if (!this.folders.routers)
      throw new Error(texts.folders_routers_is_required);
  }

  writeInfo() {
    this.log(`${colors.underline('Name')}: ${colors.blue(config.name)}`);
    this.log(`${colors.underline('Version')}: ${config.version}`);
    this.log(`${colors.underline('Description')}: ${config.description}`);
    this.log(`${colors.underline('Author')}: ${config.author}`);
    this.log(`${colors.underline('Github')}: ${config.repository.url}`);
    this.log('');
  }
      ...
      this.app[route.method](
        ...
        route.middlewares.map((middleware) => {
          return typeof middleware === 'function'
            ? middleware
            : require(`${middlewaresFolder}/${middleware}`);
        }),
        action
      );
    }
